...
def watch_price(symbol):
    ...
    def on_message(ws, message):
        ...
        diff = (price - bitvavo_usd) / bitvavo_usd
        print(f"{coin}: Binance={price} / Bitvavo={bitvavo_usd:.4f} / Diff={diff*100:.2f}%")
        if 0.05 <= diff <= 0.2:
            if not r.sismember(ALERTED_KEY, coin):
                send_message(f"اشتري {coin} يا توتو sniper")
                r.sadd(ALERTED_KEY, coin)
...
def fetch_bitvavo_top_symbols():
    try:
        ...
        return [coin["market"].replace("-EUR", "") for coin in sorted_coins[:10]]
    except:
        return []
